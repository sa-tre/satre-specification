...
No.
The SATRE specification defines a set of stakeholder {ref}`roles <satre_roles>` and feature [capabilities](https://glossary.uktre.org/en/stable/#term-capability) for TREs, which were decided according to our {ref}`architectural principles <satre_principles>`.
