...
:::{note}
Throughout this document, we will use the term "{ref}`TRE operator <admin_roles>`" to refer to the team running a particular TRE.
:::
...
Optional
: We believe many TREs would benefit from this component. However, we recognise there are reasons a {ref}`TRE operator <admin_roles>` may actively choose not to support this component.
:::

{ref}`TRE operator <admin_roles>`s are able to demonstrate that they meet the specification by showing they can fulfil all **mandatory** components.
...
: The {term}`principles <architectural principle>` that all {ref}`TRE operator <admin_roles>`s looking to use the specification should hold themselves accountable to.
...
Together, these provide a framework that {ref}`TRE operator <admin_roles>`s can measure themselves against.
...
Finally, the {ref}`TRE operator <admin_roles>`s will need to consider different {ref}`roles <satre_roles>` with which individuals might interact with the TRE.
...
: What the {ref}`TRE operator <admin_roles>`s do to ensure information risk is measured and managed to an acceptable level.

{ref}`Computing technology <pillar_computing_technology>`
: What the {ref}`TRE operator <admin_roles>`s do to manage systems for storing, retrieving, and sending information.

{ref}`Data management <pillar_data_management>`
: What the {ref}`TRE operator <admin_roles>`s do to manage data assets and ensure information remains secure.

In addition to these capabilities, any {ref}`TRE operator <admin_roles>` will need to possess various {ref}`supporting capabilities <pillar_supporting>`.
